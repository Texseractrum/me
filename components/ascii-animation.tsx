...
export default function ASCIIAnimation({
  ...
  const loadAllFrames = useCallback(async (resolvedSource = sourceRef.current) => {
    ...
    if (resolvedSource.format === "color") {
      ...
      const loaded = await Promise.all(
        Array.from({ length: resolvedMeta.frameCount }, async (_, i) => {
          const response = await fetch(`${resolvedSource.baseUrl}/frame_${String(i + 1).padStart(5, "0")}.bin`)
  ...
  const currentTextFrame = frames[currentFrame] || frames[0] || ""
  ...
  useEffect(() => {
    ...
    return () => observer.disconnect()
  }, [currentTextFrame, currentFrame, format, meta])
